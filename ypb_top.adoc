...
|`aid`
|M→S
|Request ID. Used to match responses (`rid`) to requests when multiple transactions are in flight, mandatory to support multiple outstanding requests.
...
** `rdata` shall be fixed to `'0` by the slave, may be ignored by the master.
** Write completion and/or errors may still be signaled with `rvalid`/`err`.
* If the interface is not used in VIPT mode:
** `vreq = 0`, `vaddr` is don’t-care, `vgnt` may be ignored by the master, `vreq` may be ignored by the slave.
...
** `aid` may be ignored by the slave
** `rid` may be ignored by the master
...
** In *P2P‑VIPT*: they shall be driven and valid and stable on the cycle where `vreq = 1` until `vgnt = 1` or `kill_request = 1`.
** In *P2P‑PhysOnly* and *BUS*: they shall be driven and valid and stable on the cycle where `preq = 1` until `pgnt = 1`.
* Data‑ and access‑type attributes (`wdata`, `cacheable`, `access_type`, plus `we` and `atop`) are associated with the **physical phase**:
** In all modes, they shall be driven and valid and stable on the cycle where `preq = 1` until `pgnt = 1`.

TO BE DEFINE: this ensures compatibility with CRI of HPdcache, there is probably no need to have `be`, `size` and `aid` at the initial request in *P2P‑VIPT* mode. 
...
* When virtual phase is granted by slave, master shall sent Physical Phase at the next cycle, if master is not ready (translation not ready), access need to be killed by sending `kill_request = 1`
